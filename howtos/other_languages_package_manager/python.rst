...
The recipe now declares 2 ``requires`` that we will be using to create the binary extension: the **Poco library** and the **pybind11 library**.
...
- The ``build()`` method is actually used to invoke CMake. You may see that we had to hardcode the Python path in the example, as the *CMakeLists.txt*
  call to ``find_package(PythonLibs)`` didn't find my Python installation in *C:/Python27*, even though that is a standard path. I have alsoi added the ``cmake``
