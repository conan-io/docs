...
In the GCC 5.1 release libstdc++ introduced a `new library ABI <https://gcc.gnu.org/onlinedocs/libstdc++/manual/using_dual_abi.html>`_
...
When Conan create the default profile the first time it runs, adjust the ``compiler.libcxx`` setting to
``libstdc++`` for backwards compatibility. If you are using GCC >= 5, your compiler is likely using
