<a id="creating-packages-other-prebuilt"></a>
...
>    **conan create**. This method will keep your local project build if you are using an IDE.
...
1. We call **conan install** to generate the `conan_toolchain.cmake` file and the `CMakeUserPresets.json`
...
Now, for each different configuration, we run the **conan export-pkg** command. Later, we can list the binaries
...
built correctly, and then upload it to a Conan remote with **conan upload**.
...
package could be to define a **build_policy="missing"**, especially if the URLs are also
...
#### SEE ALSO
- [JFrog Academy Conan 2 Essentials Module 3, Lesson 15: Creating a Recipe For Prebuilt Binaries](https://academy.jfrog.com/path/conan-cc-package-manager/conan-2-essentials-module-3-advanced-package-creation-scenarios?utm_source=Conan+Docs)
