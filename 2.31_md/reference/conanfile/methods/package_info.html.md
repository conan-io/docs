<a id="reference-conanfile-methods-package-info"></a>
...
<a id="conan-conanfile-model-cppinfo"></a>

<a id="conan-conanfile-model-cppinfo-attributes"></a>
...
There is one exception that would allow using absolute paths in `self.cpp_info.xxxxdirs` fields: for recipes that serve as a “wrapper” of a library installed in the system, but not by Conan. See the [example of a recipe wrapping a library installed in the system](https://docs.conan.io/2//examples/tools/system/system_package/package_manager.html.md#examples-tools-system-package-manager-wrapper).
...
- **cflags**, **cxxflags**, **sharedlinkflags**, **exelinkflags**: List of flags that the consumer should activate for proper behavior.
  Rarely used. These flags can be handled by the [compiler flags plugin](https://docs.conan.io/2//reference/extensions/compiler_flags_plugin.html.md#reference-extensions-compiler-flags-plugin) when mixing different
...
def requirements(self):
    ...
    self.requires("another/1.0")  # not linked or embedded in this package in any way
...
changes in that dependency, like building it with a different `shared` value, could break the package without Conan being able to
detect it.

<a id="reference-conanfile-methods-package-info-buildenv-info"></a>

<a id="reference-conanfile-methods-package-info-runenv-info"></a>
...
<a id="conan-conanfile-model-conf-info"></a>

<a id="conan-conanfile-model-conf-info-tool-requires"></a>
...
<a id="conan-conanfile-model-generator-info"></a>
...
#### SEE ALSO
See [the defining package information tutorial](https://docs.conan.io/2//tutorial/creating_packages/define_package_information.html.md#tutorial-creating-define-package-info) for more information.
