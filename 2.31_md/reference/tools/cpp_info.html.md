...
The `self.cpp_info` object in the `package_info()` is a `CppInfo` object, so in most cases it will not be necessary to explicitly instantiate it, and using it as explained in [the package_info()](https://docs.conan.io/2//reference/conanfile/methods/package_info.html.md#reference-conanfile-methods-package-info) section would be enough.
...
The public documented interface (besides the defined one in [the package_info()](https://docs.conan.io/2//reference/conanfile/methods/package_info.html.md#reference-conanfile-methods-package-info)) is:
...
The `save()/load()` operations shouldn’t be needed in recipes in most cases either, only for some exceptional circumstances like the [PkgConfig fill_cpp_info example](https://docs.conan.io/2//examples/tools/gnu/pkg_config_sysroot.html.md#examples-tools-gnu-pkg-config-sysroot).
