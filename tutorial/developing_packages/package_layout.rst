...
Defines the ``say`` project structure for the source code and the folders where the files
generated by Conan and built artifacts will be located. This structure is independent of
wether the package is in editable mode or exported and built in the Conan local cache.
Let's define our folder structure for the ``say`` package:
...
- We set ``self.folders.build`` to be **./build/Release** or **./build/Debug** depending
  on the ``build_type`` setting. Those are the folders where we want the built binaries to
  be located.
...
First, we set the information for **cpp.package**. This defines the contents of the
package and its location relative to the folder where the package is stored in the local
cache. Please, note that defining this information is equivalent to defining
``self.cpp_info`` in the ``package_info()`` method. This is the information we defined:
...
you check in the ``[YOUR_CONAN_HOME]/p`` folder, you will find that two new folders
similar to these were created:
...
- ``self.cpp.build.libdirs`` set to ``["."]``. This location is relative to the
  ...
  editable packages this location will point to **<local_folder>/build/<build_type>**. We
  add the ``include`` folder.

To check how this information affects consumers we are going to do first put the ``say``
...
.. seealso::
    ...
    - Define the layout() :ref:`when you ave the conanfile in a subfolder<examples_conanfile_layout_conanfile_in_subfolder>`
