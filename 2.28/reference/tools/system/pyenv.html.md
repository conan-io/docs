<a id="conan-tools-system-pyenv"></a>
...
To use a different one, you can set a Python path in the `tools.system.pyenv:python_interpreter` [configuration](https://docs.conan.io/2//reference/config_files/global_conf.html.md#reference-config-files-global-conf).

### *class* PyEnv(conanfile, folder=None, name='', py_version=None)
...
* **Parameters:**
  ...
  * **py_version** – Optional python version to create the virtualenv using UV
...
* **Parameters:**
  ...
  * **pip_args** – additional argument list to be passed to the ‘pip install’ command,
    e.g.: [’–no-cache-dir’, ‘–index-url’, ‘[https://my.pypi.org/simple](https://my.pypi.org/simple)’].
    Defaults to `None`.
* **Returns:**
