<a id="conan-tools-gnu-pkgconfigdeps"></a>
...
You can change that default behavior with the `pkg_config_name` and `pkg_config_aliases` properties. See [Properties section below](#pkgconfigdeps-properties).
...
Using this method you can overwrite the [property](#pkgconfigdeps-properties) values set by
...
* **Parameters:**
  * **dep** – Name of the dependency to set the [property](#pkgconfigdeps-properties). For
    components use the syntax: `dep_name::component_name`.
  * **prop** – Name of the [property](#pkgconfigdeps-properties).
...
<a id="pkgconfigdeps-properties"></a>
...
- **pkg_config_custom_content** property will add user defined content to the  *.pc* files created by this generator as freeform variables.
  ...
  Click [here](https://people.freedesktop.org/~dbn/pkg-config-guide.html#concepts) for more information about the type of variables in a `*.pc` file.
