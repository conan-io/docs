...
* **Parameters:**
  ...
  * **pattern** – (Required) An fnmatch file pattern, or a list/tuple of fnmatch patterns,
...
### replace_in_file(conanfile, file_path, search, replace, strict=True, encoding='utf-8', regex=False, flags=RegexFlag.MULTILINE)
...
* **Parameters:**
  ...
  * **search** – String you want to be replaced. With `regex=True`, treated as a regular
    expression.
  * **replace** – String to replace the searched string. With regex mode, backreferences
  ...
  * **encoding** – (Optional, Defaulted to utf-8): Specifies the input and output files text
    encoding.
  * **regex** – (Optional, Defaulted to `False`) If `True`, `search` is treated as a
    regular expression and applied with `re.sub`.
  * **flags** – (Optional, Defaulted to `re.MULTILINE`) Flags passed to `re.sub` when
    `regex=True`.
* **Returns:**
