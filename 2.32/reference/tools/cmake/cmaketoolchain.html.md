<a id="conan-cmake-toolchain"></a>

<a id="conan-tools-cmaketoolchain"></a>
...
- **CMakePresets.json**: This toolchain generates a standard CMakePresets.json file. For
  more information, refer to the documentation [here](https://cmake.org/cmake/help/latest/manual/cmake-presets.7.html). It currently uses
  ...
  >       [VirtualBuildEnv](https://docs.conan.io/2//reference/tools/env/virtualbuildenv.html.md#conan-tools-env-virtualbuildenv), if applicable. This
  ...
  >   : - The configurePreset associated with this build preset.
  >     - An environment section, setting all the environment information related to the
  >       [VirtualRunEnv](https://docs.conan.io/2//reference/tools/env/virtualrunenv.html.md#conan-tools-env-virtualrunenv), if applicable. This
  >       environment can be modified in the generate() method of the recipe by passing
...
- **CMakeUserPresets.json**:  If you declare a `layout()` in the recipe and your
  ...
  See [extending your own CMake presets](https://docs.conan.io/2//examples/tools/cmake/cmake_toolchain/extend_own_cmake_presets.html.md#examples-tools-cmake-toolchain-build-project-extend-presets) for a full example.
...
<a id="conan-cmake-toolchain-cache-variables"></a>
...
applied with `-D` arguments when calling `cmake.configure` using the [CMake() build helper](https://docs.conan.io/2//reference/tools/cmake.html.md#conan-tools-cmake).
...
[cache_variables](#conan-cmake-toolchain-cache-variables) is what you probably want to
...
change the name and location of the `CMakeUserPresets.json` file. Assigning it to an empty string will disable the generation of the file. Please check the [conf section](#conan-cmake-toolchain-conf) for more information.

If the value assigned to `tools.cmake.cmaketoolchain:user_presets` contains a directory component, e.g.
`presets/CMakeUserPresets.json`, Conan will rely on the CMake `--preset-file` mechanism (available since
CMake 4.4) to let the user presets file live outside the CMake root directory. In that case, the commands suggested by Conan to configure and
build the project will be like:

```bash
$ cmake --preset-file presets/CMakeUserPresets.json --preset conan-release
```
...
[Environment](https://docs.conan.io/2//reference/tools/env/environment.html.md#conan-tools-env-environment-model). This can be accomplished in the
generate() method.
...
<a id="conan-cmake-user-toolchain"></a>
...
- **apple_system**: Defines `CMAKE_OSX_ARCHITECTURES` (see the [universal binaries section](#conan-tools-cmaketoolchain-universal-binaries)), `CMAKE_OSX_SYSROOT` for Apple systems. It also
...
With the new `CMakeConfigDeps` generator, the `conan_toolchain.cmake` block `find_paths` no longer
...
<a id="conan-tools-cmaketoolchain-universal-binaries"></a>
...
<a id="conan-cmake-toolchain-conf"></a>
...
- **tools.cmake.cmaketoolchain:user_presets**: (Experimental) Allows setting a custom name or subfolder for the CMakeUserPresets.json file. An empty string disables file generation entirely. A relative path with a directory component enables the CMake >= 4.4 `--preset-file` mode.
