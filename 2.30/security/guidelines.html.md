...
- Enable dependencies vulnerability checking with `conan audit`, check [the conan audit docs](https://docs.conan.io/2//security/audit.html.md#security-audit)
- In many production cases, it is very recommended to fully own the SW lifecycle (SWLC) of the dependencies, including the third
  party dependencies. For this reason, the [Using ConanCenter packages in production environments](https://docs.conan.io/2//devops/using_conancenter.html.md#devops-consuming-conan-center) section recommends building your own binaries
  from source and storing those binaries in your own private server, without downloading packages directly from ConanCenter.
  The [local-recipes-index feature](https://docs.conan.io/2//devops/devops_local_recipes_index.html.md#devops-local-recipes-index) was designed to help in this process.
- To avoid being disrupted by internet outages and possible tampering of tarballs downloaded from the internet, the
  [Backup sources](https://docs.conan.io/2//devops/backup_sources/sources_backup.html.md#conan-backup-sources) feature can be used.
