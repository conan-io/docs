<a id="creating-packages-create-your-first-conan-package"></a>
...
the valid settings and options for the package. As we explained in the [consuming
packages section](https://docs.conan.io/2//tutorial/consuming_packages/different_configurations.html.md#settings-and-options-difference):
...
* `options` are package-specific configuration and can be defaulted in recipes. In this case, we
  ...
  recommended to define a explicit `package_type` see [package_type reference](https://docs.conan.io/2//reference/conanfile/attributes.html.md#reference-conanfile-attributes-package-type).
...
* The `package()` method copies artifacts (headers, libs) from the build folder to the
  ...
  didn’t implement it, it would be easy to write an equivalent using the [copy()
  tool](https://docs.conan.io/2//reference/tools/files/basic.html.md#conan-tools-files-copy) in the `package()` method.
...
* A new build from source for the `hello/1.0` package starts, calling the
  ...
  in the Conan cache.
* Conan then moves to the *test_package* folder and executes a **conan install** +
...
The **conan create** command receives the same parameters as **conan install**, so
...
When you did the **conan create** command, the build of your package did not take
...
You already used the **conan list** command to list the recipes and binaries stored
in the local cache.
...
#### SEE ALSO
- [JFrog Academy Conan 2 Essentials Module 2, Lesson 8: Creating Your First Conan Package](https://academy.jfrog.com/path/conan-cc-package-manager/conan-2-essentials-module-2-package-creation-and-uploading?utm_source=Conan+Docs)
- [Create your first Conan package with Visual Studio/MSBuild](https://docs.conan.io/2//examples/tools/microsoft/msbuild/create_your_first_package.html.md#examples-tools-microsoft-create-first-package).
- [Create your first Conan package with Meson](https://docs.conan.io/2//examples/tools/meson/create_your_first_package.html.md#examples-tools-meson-create-first-package).
- [Create your first Conan package with Autotools (only Linux)](https://docs.conan.io/2//examples/tools/autotools/create_your_first_package.html.md#examples-tools-autotools-create-first-package).
- [CMake built-in integrations reference](https://docs.conan.io/2//reference/tools/cmake.html.md#conan-tools-cmake).
- [conan create command reference](https://docs.conan.io/2//reference/commands/create.html.md#reference-commands-create) and [Conan list command reference](https://docs.conan.io/2//reference/commands/list.html.md#reference-commands-list).
