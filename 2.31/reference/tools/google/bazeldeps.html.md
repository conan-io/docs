...
that, it also generates Bazel 6.x compatible file like *dependencies.bzl*, Bazel >= 7.1 compatible files
like *conan_deps_module_extension.bzl* and *conan_deps_repo_rules.bzl*, and Bazel >= 7.2 / 9.x compatible
files like *conan_deps.MODULE.bazel* and *BUILD.rules_cc.bazel* (with explicit `rules_cc` `load()`
statements required by Bazel 9.x). All of them contain the logic to load all your Conan dependencies
through your *WORKSPACE* | *MODULE.bazel*.
...
├── BUILD.bazel
├── conanfile.py
├── conan_deps.MODULE.bazel
├── conan_deps_module_extension.bzl
├── conan_deps_module_extension_rules_cc.bzl
├── conan_deps_repo_rules.bzl
├── dependencies.bzl
└── zlib
    ├── BUILD.bazel
    └── BUILD.rules_cc.bazel
```
...
* *zlib/BUILD.rules_cc.bazel*: same targets as *BUILD.bazel*, but with `rules_cc` `load()` statements
  (Bazel 9.x compatible).
...
* *conan_deps_module_extension_rules_cc.bzl*: (Bazel >= 7.2 / 9.x compatible) module extension that references
  the *BUILD.rules_cc.bazel* dependency files.
* *conan_deps.MODULE.bazel*: (Bazel >= 7.2 / 9.x compatible) generated snippet to include in your *MODULE.bazel*
  file. Declares the `rules_cc` dependency and loads the module extension above.
...
**Bazel >= 7.2 compatible (recommended for Bazel 9.x)**

```python
include("//:conan_deps.MODULE.bazel")
```

#### IMPORTANT
Bazel 9.x requires C++ rules to be loaded from `rules_cc`. Include the generated
*conan_deps.MODULE.bazel* snippet in your *MODULE.bazel* file — it declares the `rules_cc`
dependency and uses the *BUILD.rules_cc.bazel* dependency files.
...
#### NOTE
For Bazel 9.x, the equivalent *BUILD.rules_cc.bazel* file includes `load("@rules_cc//cc:cc_import.bzl", "cc_import")`
and `load("@rules_cc//cc:cc_library.bzl", "cc_library")` at the top.
