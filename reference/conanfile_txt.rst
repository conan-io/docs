...
The ``conanfile.txt`` is a simplified version of ``conanfile.py``, aimed at simple consumption of dependencies, but it
...
but it requires using a ``conanfile.py``, read the ``CMakeDeps`` reference for more information).
