...
- ``tools.build:cflags`` list of extra pure C flags that will be used by ``CL`` and ``CFLAGS``.
- ``tools.build:cxxflags`` list of extra C++ flags that will be used by ``CL``, ``CPPFLAGS`` and ``CXXFLAGS``.
...
If your ``Makefile`` script needs some other environment variable rather than ``CL``, ``_LINK_``, ``CFLAGS``,
``CPPFLAGS`` or ``CXXFLAGS``, you can customize it before calling the ``generate()`` method.
