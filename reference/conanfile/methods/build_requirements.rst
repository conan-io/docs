...
Recall that ``tool_requires`` are intended exclusively for depending on tools like ``cmake`` or ``ninja``, that run in the "build" context, but not for library like dependencies that would be linked into binaries. For libraries or library-like dependencies, use ``requires`` or ``test_requires``.
