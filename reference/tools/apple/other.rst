...
The locations of the ``otool`` and ``install_name_tool`` executables can be specified via
the ``tools.apple:otool_program`` and ``tools.apple:install_name_tool_program`` ``[conf]``
variables.
