...
It's also possible to generate Fish-compatible ``.fish`` scripts instead of ``.sh`` ones, using the
...
create a ``conanbuild.bat|sh|ps1|fish`` file aggregating all the launchers in order. The ``conanbuild.sh|bat|ps1|fish``
