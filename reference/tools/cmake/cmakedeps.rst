...
- **cmake_set_interface_link_directories**: (**deprecated** in 2.14). It was used for ``#pragma comment(lib, "foo")`` time ago, but it is no longer necessary for ``CMakeDeps`` generator. In the new ``CMakeConfigDeps`` generator, it will be necessary to correctly specify the library details in the ``cpp_info``, aligned with the CPS practices.
