...
- **configure** (args=None, defs=None, source_dir=None, build_dir=None)
    ...
    - **source_dir**: CMake's source directory where ``CMakeLists.txt`` is located. The default value is the ``build`` folder if ``None`` is specified (or the ``source`` folder if ``no_copy_sources`` is specified). Relative paths are allowed and will be relative to ``build_dir``
